...
if(isset($_POST["reg_submit"])) {
        ...
        if (empty($_POST["address"])) {
            ...
            //     echo $addrErr;
            // }
             if (empty($_POST["T_mode"])) {
            $addrErr = "transaction mode is required";
            $flag=1;
            echo $t_modeErr;
        } else {
            $T_mode = test_input($_POST["T_mode"]);
        }
        //CONTACT VALIDATION
        ...
        if($flag == 0)
        {
            ...
            $sql = "INSERT INTO user (`name`,`email`,`phone`,`pass`,`address`,`T_mode`)
                    VALUES('$name','$email','$contactNo','$password','$address','$T_mode')";
                    echo $sql;
...
<form action="signup.php" method="post" class="form-horizontal" role="form" onsubmit="return validateForm()">
    ...
    <div class="form-group">
        <div class="col-md-12">
            <input type="text" class="form-control" name="T_mode" placeholder="Transaction MODE" required>
            <!-- <label><?php echo $addrErr;?></label> -->
        </div>
    </div>
    <div class="form-group">
